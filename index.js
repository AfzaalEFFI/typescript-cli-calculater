...
const wellcome = async () => {
    let welcomeText = chalkAnimation.rainbow("** =========== Welcome To My CLI Calculator =========== **");
    await sleep();
    welcomeText.stop;
 ...
 `));
    console.log(chalk.greenBright(figlet.textSync("Develop By Afzzal", {
        font: "Big",
...
async function calcualtion() {
    ...
    if (ans.operation === "Addition") {
        console.log(chalk.greenBright(`${ans.first_num}+${ans.secand_num}= ${ans.first_num + ans.secand_num}`));
    }
    else if (ans.operation === "Subtraction") {
        console.log(chalk.greenBright(`${ans.first_num} - ${ans.secand_num}=${ans.first_num - ans.secand_num}`));
    }
    else if (ans.operation === "Multiplication") {
        console.log(`${ans.first_num} * ${ans.secand_num}= ${ans.first_num * ans.secand_num}`);
    }
